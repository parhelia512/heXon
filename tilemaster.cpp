  ...
  masterControl_{masterControl}
{
    ...
    for (int i = 0; i < bigHexSize; i++) {
        for (int j = 0; j < bigHexSize; j++) {
            if (i < (bigHexSize - bigHexSize / 4) + j / 2 &&  //Exclude bottom right
                ...
                Vector3 tilePos = Vector3((-bigHexSize / 2.f + i) * 2.f + j % 2, -0.1f, (-bigHexSize / 2.f + j + 0.5f) * 1.8f);
    ...
    playLight->SetRange(10.f);
    playLight->SetColor(Color(1.f, 0.9f, 0.95f));
...
WeakPtr<Tile> TileMaster::GetRandomTile()
{
    ...
    if (tiles.Size()){
        ...
        while (!tile){
            ...
            Ray spawnRay(tryTile->rootNode_->GetPosition()-Vector3::UP, Vector3::UP*10.f);
            if (!masterControl_->PhysicsRayCast(hitResults, spawnRay, 23.f, M_MAX_UNSIGNED)){
                tile = tryTile;
            }
        }
